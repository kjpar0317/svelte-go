<script lang="ts">
	...
	let currCoin = 'BTC';
	let sResults: any = null;
	let aResults: any = null;
	...
	async function handleSelectCoinAiAnalysis(val: string, bNoLoading: boolean) {
		...
		if (sResults !== null && sResults.RelatedArticles.length > 0) {
			...
			let categories: any[] = [];
			let seriesPositive: any[] = [];
			let seriesNegative: any[] = [];

			sResults.RelatedArticles.forEach((f: any, index: number) => {
				if (f.Sentiment.positive.length > 0 || f.Sentiment.negative.length > 0) {
	...
	$: handleSelectCoinAiAnalysis(currCoin, false);
